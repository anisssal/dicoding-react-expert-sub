...
export const asyncPostThread = createAsyncThunk('thread/new', async ({ title, body, category }, { rejectWithValue }) => {
  try {
...
export const asyncToggleUpVotedThread = createAsyncThunk('thread/upVote', async ({ id, authUserId }, { dispatch, getState, rejectWithValue }) => {
...
export const asyncToggleDownVotedThread = createAsyncThunk('thread/downVote', async ({ id, authUserId }, { dispatch, getState, rejectWithValue }) => {
  const threadsState = getState().threads;
  const thread = threadsState.threads.find((t) => t.id === id);
  const isUpVoted = thread.upVotesBy.includes(authUserId);
  const isDownVoted = thread.downVotesBy.includes(authUserId);
